...
export class PaymentService {
  ...
  async processPayment(paymentData: PaymentRequest): Promise<PaymentResponse> {
    ...
    try {
      ...
      if (!sessionDetails.sourceOfFunds?.provided?.card) {
        return {
          success: false,
          error: 'Card details not found in session',
          message: 'Please enter your card details and try again',
        };
      }

      // Step 1: Initiate 3DS authentication
      const authTransactionId = `AUTH-${uuidv4()}`;
      const authRequest = {
      ...
      console.log('Auth URL:', `/order/${orderId}/transaction/${authTransactionId}`);
      console.log('Auth Payload:', JSON.stringify(authRequest, null, 2));

      let authResponse;
      try {
        authResponse = await this.gatewayClient.put(
          `/order/${orderId}/transaction/${authTransactionId}`,
          authRequest
        );
        console.log('🔐 Auth response:', JSON.stringify(authResponse.data, null, 2));
      } catch (authError: any) {
        console.log('⚠️ 3DS initiation not supported or failed, proceeding with direct payment');
        console.log('Auth error:', authError.response?.data);
        // If 3DS initiation fails, try direct payment
      ...
      console.log('🔐 Gateway recommendation:', gatewayRecommendation);
      console.log('🔐 Auth result:', authResult);

      if (gatewayRecommendation === 'PROCEED' || authResult === 'SUCCESS') {
        // Authentication successful - proceed with payment
        console.log('✅ 3DS authentication successful or not required - proceeding with payment');
        return await this.executePayment(orderId, sessionDetails, paymentData);
      } else if (authResponse.data?.authentication?.redirect) {
        // 3DS challenge required
        console.log('🔄 3DS challenge required');
        const redirectData = authResponse.data.authentication.redirect;
        
        return {
          success: false,
          require3DS: true,
          redirectHtml: redirectData.html || this.generate3DSRedirectHtml(redirectData),
          orderId: orderId,
        ...
        console.log('⚠️ Unclear 3DS status, attempting payment');
        return await this.executePayment(orderId, sessionDetails, paymentData);
      }
  ...
  /**
   * Generate 3DS redirect HTML if not provided
   */
  private generate3DSRedirectHtml(redirectData: any): string {
    const { url, method = 'POST', parameters = {} } = redirectData;
    
    let formHtml = `
      <html>
        <head><title>3D Secure Authentication</title></head>
        <body>
          <form id="3dsForm" method="${method}" action="${url}">
    `;
    ...
    formHtml += `
          ...
          <script>document.getElementById('3dsForm').submit();</script>
        </body>
      </html>
    `;
  ...
  private async executePayment(orderId: string, sessionDetails: any, paymentData: PaymentRequest): Promise<PaymentResponse> {
    ...
    try {
      ...
      await this.savePaymentHistory({
        ...
        orderId: orderId,
        sessionId: paymentData.sessionId,
        amount: gatewayResponse.transaction.amount,
